...
```python clean_train.py --knowledge_type top \\
...
def parse_args():
    ...
    #     type=str,
    #     default="top",
    #     choices=["top", "bot", "rand"],
    #     help="Pass in a knowledge type.",
    # )
    # parser.add_argument(
    #     "--knowledge_graph",
    #     type=str,
    #     default="conceptnet",
    #     choices=["conceptnet", "stereokg"],
    #     help="Pass in a knowledge type.",
    # )
    ...
    parser.add_argument(
        ...
        default="sbic",
        choices=["sbic", "latent"],
        help="Pass in a dataset type.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="model",
        help="Pass in a model output directory.",
    )

    parser.add_argument("--seed", type=int, default=685, help="Pass in a seed value.")
    parser.add_argument(
        "--batch_size", type=int, default=4, help="Pass in a batch size."
    )
    # parser.add_argument("--k", type=int, default=0, help="Pass in a value for k.")
    parser.add_argument(
        "--num_epochs",
        type=float,
        default=3.0,
        help="Pass in the number of training epochs.",
    )
    parser.add_argument(
        "--lr", type=float, default=5e-5, help="Pass in the learning rate for training."
    )
    parser.add_argument(
        "--warmup_ratio",
        type=float,
        default=0.5,
        help="Warmup ratio to be applied if training for 1 epoch.",
    )
    parser.add_argument(
        "--sep", type=str, default="\t", help="Separator for data file."
    )
    parser.add_argument(
        "--data_file",
        type=str,
        default="../data/SBIC.v2.trn.csv",
        help="Data File to load.",
    )
    parser.add_argument(
        "--dev_file", type=str, help="Dev File to load in case data split isn't used."
    )  # we do not use the dev file here, but ../data/SBIC.v2.dev.csv can be used
...
if __name__ == "__main__":
    args = parse_args()
    if SEQ2SEQ_MODEL_NAME == None:
        SEQ2SEQ_MODEL_NAME = args.model
    print(SEQ2SEQ_MODEL_NAME)
    check_args(args)
    print(args)
    set_seed(args.seed)

    datasets = get_processed_data(args)
    ...
    data_collator = DataCollatorForSeq2Seq(
        tokenizer, model, padding=True, max_length=MAX_LENGTH
    )
    train(
        model,
        tokenized,
