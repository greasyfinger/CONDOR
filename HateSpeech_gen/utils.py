...
def generate_stereotype(
    ...
    output_ids = model.generate(
        batch["input_ids"], num_beams=num_beams, length_penalty=5.0, **model_kwargs
    )
...
def generate_scores(
    ...
    if not (save_results_to_csv):
        ...
        metric = load_metric("bertscore", trust_remote_code=True)
        bert_scores = metric.compute(
        ...
        print(
            "Rouge Score (Avg) (Precision, Recall, F1): ",
            [round(i, 4) for i in rouge_scores_avg],
        )
        print(
            "Rouge Score (Max) (Precision, Recall, F1): ",
            [round(i, 4) for i in rouge_scores_max],
        )
        print(
            "BERT Score (Max) (Precision, Recall, F1): ",
            [round(i, 4) for i in bert_score],
        )
...
class MinibatchIterator:
    ...
    def __next__(self):
        ...
        next_batch = self.tokenizer.pad(
            next_batch, padding=True, max_length=self.max_length
        )
...
def get_step_variables(
    num_rows, num_epochs, batch_size, eval_percent=5.0, warmup_ratio=0.5
):
    if num_epochs == 1:
    ...
    if num_epochs == 1:
        warmup_steps = int(one_epoch_steps * warmup_ratio)
    else:
...
def tokenize_data(
    ...
    tokenizer = BartTokenizer.from_pretrained(
        "facebook/bart-base", additional_special_tokens=special_tokens
    )
    ...
    if train:
        ...
        tokenized = dataset.map(
            tokenize, batched=True, num_proc=1, remove_columns=remove_cols
        )
    else:
        tokenized = dataset.map(
            tokenize,
