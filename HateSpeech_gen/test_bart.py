...
def parse_args():
    ...
    parser.add_argument(
        "--get_ranking_scores",
        action="store_true",
        help="Pass true if you want to dump the cosine scores as well",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="Batch size for testing. Use a smaller batch size with more classifiers.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--generate_scores", action="store_true", help="If True, will generate scores"
    )
    parser.add_argument(
        "--save_results_to_csv",
        action="store_true",
        help="If true, will save the generation results to csv.",
    )
    parser.add_argument(
        "--num_results",
        type=int,
        default=200,
        help="If saving results to csv, then this variable saves 'num_results' samples.",
    )
    parser.add_argument(
        "--data_file",
        type=str,
        default="../data/SBIC.v2.tst.csv",
...
def check_args(args):
    ...
    pickle_file = os.path.join(
        PRED_DIRECTORY, args.output + "_" + model_name + "_" + data_source + ".pickle"
    )
    results_file = os.path.join(
        RESULTS_DIRECTORY, args.output + "_" + model_name + "_" + data_source + ".csv"
    )
...
if __name__ == "__main__":
    ...
    if not os.path.exists(pickle_file):
        ...
        if args.dataset_type == "sbic":
            ...
            #         emb_dict=emb_dict,
            #         k=args.k,
            #         get_scores=args.get_ranking_scores,
            #         ds=args.dataset_type,
            #     )
            df = df_post.merge(
                df.drop(columns="post"), on="HITId", validate="one_to_many"
            )
        ...
        seq2seq_tok, tokenized = tokenize_data(
            dataset, train=False, padding=False, max_length=MAX_LENGTH
        )
        ...
        batch_iter = MinibatchIterator(
            tokenized,
            seq2seq_tok,
            batch_size=args.batch_size,
            torch_cols=torch_cols,
