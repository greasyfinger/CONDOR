...
PROMPTS = [
    ...
    "worked as",
    "have a job as",
    "have a part-time job as",
    "earned money by",
    "started working as",
    "have various hobbies such as",
    "would regularly engage in",
...
class LemmaTokenizer:
    ...
    def __call__(self, doc):
        return [
            self.wnl.lemmatize(t) for t in word_tokenize(doc) if t not in self.ignore
        ]
...
def add_ordered_k_tuples(
    ...
    if kg == "conceptnet":
        ...
        if get_scores:
            with open(
                f"cosinescore_dumps/{ds}_{kg}_{knowledge_type}_k_{k}_dumps.pkl", "wb"
            ) as file:
        ...
        with open("../data/stereoKG_linearised.txt", "r") as file:
            linearised_data = file.readlines()
        linearised_data = [line.strip() for line in linearised_data]
        # df_stereokg['linearised'] = linearised_data
        # stereokg_vals = df_stereokg["linearised"].values
        stereokg_vals = pd.DataFrame(linearised_data, columns=["linearised"])[
            "linearised"
        ].values
        ...
        for i, post in enumerate(tqdm(posts)):
            ...
            stereokg_k_triples = get_cosine_k_triples(
                k=k,
                cosine_scores=cosine_scores,
                kg_vals=stereokg_vals,
                typ=knowledge_type,
            )
            cosine_top_k_scores = cosine_scores[
                cosine_scores[:, 0].cpu().sort().indices.numpy()[::-1][:k].copy()
            ]
            edge_string = ""
...
def concat_top_k_tuples(
    ...
    if knowledge_graph == "conceptnet":
        ...
        ordered_k_tuples = collect_ordered_k_tuples(
            query_lists, query_dict, all_tuples, k=k, k_type=knowledge_type
        )

    posts = add_ordered_k_tuples(
        posts,
        ordered_k_tuples,
        sep_token,
        knowledge_graph,
