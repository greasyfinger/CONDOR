...
ds_stereo = TripletBartDataset("path to StereoKG tsv file", tokenizer)
ds_comet = TripletBartDataset(
    "path to triples extracted on ConceptNet, from stereo seed terms",
    tokenizer,
)
dataset = ConcatDataset([ds_stereo, ds_comet])
...
for epoch in range(num_epochs):
    ...
    if epoch % 10 == 0:
        checkpoint_dir = f"seed_bart_stereo/epoch_{epoch}"
